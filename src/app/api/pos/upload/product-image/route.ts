...
/**
 * POST /api/pos/upload/product-image
 *
 * Accepts { productId, imageData (base64), skipDbUpdate? }
 * Returns { imageUrl }
 */
export async function POST(req: NextRequest) {
  try {
    ...
    const { productId, imageData, skipDbUpdate } = await req.json();
    if (!imageData) {
    ...
    const blobPath = `${orgId}/products/${productId || Date.now()}-${Date.now()}.webp`;

    const imageUrl = await uploadBase64ToBlob(imageData, blobPath);

    return NextResponse.json({ imageUrl });
  } catch (error: any) {
    console.error("[pos/upload/product-image POST]", error);
    return NextResponse.json(
